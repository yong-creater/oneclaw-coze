...
### 铁律七：集成能力必须先查文档再动手（禁止凭代码推断能力边界）

> **遇到集成服务的能力问题时，必须先查官方文档/Skill文档，禁止看现有代码用法推断能力！**

#### 核心原则

| 原则 | 说明 | 违反示例 |
|------|------|----------|
| 文档优先 | 涉及 SDK/API/集成能力时，必须先加载 Skill 读文档，再设计方案 | 看代码里只用了 `prompt` 参数，就断定 SDK 不支持图生图 |
| 验证再动手 | 查完文档确认能力边界后，再写实现方案，禁止边猜边写 | 没查文档就写了一套视觉LLM的曲线救国方案 |
| 最短路径 | 文档支持的能力直接用，禁止绕路造复杂方案 | SDK 原生支持 `image` 参数做图生图，却自己造了一套"视觉分析→描述注入"方案 |

#### 正确流程

```
遇到集成能力问题
  → 第一步：加载对应 Skill（如 image-generation）读官方文档
  → 第二步：确认 SDK/API 支持哪些参数和能力
  → 第三步：基于文档设计最简实现方案
  → 第四步：动手写代码
```
...
| 步骤 | ❌ 错误做法 | ✅ 正确做法 |
|------|-----------|-----------|
| 1 | 看代码只用 `prompt` 参数 → 断定不支持图生图 | 加载 `image-generation` Skill → 读 SDK 文档 |
| 2 | 写视觉LLM预分析方案（150行+新依赖） | 发现 `image` 参数支持图生图 |
| 3 | 用户截图指出原生支持 → 回过头改 | 直接传 `image` 参数（5行改动） |

**教训：2分钟查文档 > 2小时造轮子**

### 铁律八：积分消耗控制策略（禁止无意义调用模型API）

> **图片生成模型（seedream）是积分消耗大户，1次调用 ≈ 10次 LLM 调用。必须严格控制调用次数！**

#### 积分消耗等级

| 调用类型 | 单次消耗 | 等级 | 说明 |
|----------|----------|------|------|
| 图片生成 (seedream) | 🔴 极高 | **最贵** | 耗时10-30秒，积分消耗最大 |
| 图生图 (带参考图) | 🔴 极高 | **比纯文生图更贵** | 需先上传参考图再生成 |
| LLM 生成 (seed-2.0-pro) | 🟡 中等 | 旗舰模型 | 长文/复杂任务用 |
| LLM 生成 (seed-2.0-lite) | 🟢 较低 | 均衡模型 | 日常短文本用 |
| pnpm build | 🟡 中等 | 全量编译 | 合并到最终验证做 |
| HMR 热编译 | 🟢 低 | 增量编译 | 正常开发消耗 |

#### 模型调用规则（CRITICAL）

| 规则 | 说明 | 违反示例 |
|------|------|----------|
| 图片测试只测1个 | 日常开发/调试只测1个工具确认链路通，不必全部跑 | 8个工具挨个生成图片 |
| 短prompt测试 | LLM测试用短prompt（10字内）+ `streaming: false` | 发200字长prompt测试 |
| 优先lite模型 | 非必要不用pro模型测试 | 测试链路用seed-2.0-pro |
| 图生图慎用 | 只有需要验证图生图功能时才带参考图测试 | 每次都带参考图 |

#### 开发流程节省规则

| 阶段 | 规则 | 原因 |
|------|------|------|
| UI开发 | 代码改完依赖HMR，不要手动重启或build | 每次build消耗中等积分 |
| 样式迭代 | 先想清楚再改代码，避免反复修改 | 7轮导航迭代消耗大量积分 |
| 构建检查 | 合并到最终验证阶段做，不要改一点就build | build一次=多次HMR |
| 接口测试 | 日常只测1个生成工具，最终交付前才全量测试 | 图片生成最贵 |
| LLM测试 | 短prompt + non-streaming + lite模型 | 减少token消耗 |

#### 正确的测试流程

```
日常开发阶段：
  → 代码静态检查（pnpm lint / ts-check）  ← 零消耗
  → 服务存活探测（curl -I）                ← 零消耗
  → 前台API测试（只测非生成类接口）         ← 零消耗
  → 生成类API：只测1个工具确认链路通        ← 最少消耗

最终交付阶段：
  → 全量API测试（包括所有生成工具）         ← 必须做但只做1次
  → pnpm build                             ← 只做1次
  → 日志健康检查                           ← 零消耗
```
...
| 禁止 | 原因 |
|------|------|
| 开发调试阶段8个工具全部生成图片 | 8次图片 ≈ 80次LLM的积分 |
| 用长prompt测试LLM | 浪费token |
| 每次改代码后立即 build | 合并到最终验证 |
| 用 pro 模型做链路测试 | lite够用 |
| 反复修改同一个UI组件超过3轮 | 先想清楚再改 |
